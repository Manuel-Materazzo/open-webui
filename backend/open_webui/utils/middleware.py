...
def _parse_arg_value(val_str: str) -> Any:
    val_str = val_str.strip()
    if not val_str:
        return ''
    try:
        return JSONCodec.loads(val_str)
    except Exception:
...
def extract_tool_calls_from_text(text: str, available_tools: dict | set | list | None = None) -> list[dict]:
    ...
    Handles XML-style tool calls generated by smaller reasoning models:
        <tool_call>search_web
        <arg_key>query</arg_key>
        <arg_value>BrowseComp web search benchmark LLM</arg_value>
        <arg_key>count</arg_key>
        <arg_value>10</arg_value>
        </tool_call>
    as well as JSON payloads embedded in <tool_call> tags or code blocks.
    """
    ...
    if available_tools:
        if isinstance(available_tools, dict):
            for k in available_tools.keys():
                tools_lookup[k] = k
                tools_lookup[k.lower()] = k
        elif isinstance(available_tools, (list, set, tuple)):
    ...
    def resolve_tool_name(name: str) -> str:
        ...
        if name in tools_lookup:
            return tools_lookup[name]
        if name.lower() in tools_lookup:
            return tools_lookup[name.lower()]
        for prefix in ('functions.', 'tools.', 'builtin.'):
            if name.startswith(prefix) and name[len(prefix) :] in tools_lookup:
                return tools_lookup[name[len(prefix) :]]
        return name

    results = []

    # 1. Match XML tags like <tool_call>...</tool_call>, <tool-call>...</tool-call>, <function_call>...</function_call>
    ...
    for m in matches:
        ...
        # Attribute on tag: e.g. name="search_web"
        attr_name_match = re.search(r'(?:name|function|tool)\s*=\s*["\']([^"\']+)["\']', attrs, re.IGNORECASE)
        if attr_name_match:
            tool_name = attr_name_match.group(1).strip()

        # Try JSON in body first
        json_parsed = False
        body_trimmed = body
        first_brace = body_trimmed.find('{')
        last_brace = body_trimmed.rfind('}')
        if first_brace != -1 and last_brace != -1 and last_brace > first_brace:
            json_str = body_trimmed[first_brace : last_brace + 1]
            try:
                data = JSONCodec.loads(json_str)
                if isinstance(data, dict):
                    extracted_name = data.get('name') or data.get('function') or data.get('tool')
                    if isinstance(extracted_name, dict):
                        extracted_name = extracted_name.get('name')
                    ...
                    if isinstance(extracted_args, dict):
                        params = extracted_args
                        json_parsed = True
                    elif isinstance(extracted_args, str):
                        try:
                            params = JSONCodec.loads(extracted_args)
                            json_parsed = True
                        except Exception:
                            params = extracted_args
                            json_parsed = True
                    elif not extracted_name and not tool_name:
                        params = data
                        json_parsed = True
            except Exception:
                pass
        ...
        if not json_parsed:
            # Check for <name>...</name> or <function>...</function>
            name_tag_match = re.search(
                r'<(?:name|function|tool|tool_name|function_name)>\s*([a-zA-Z0-9_.:-]+)\s*</(?:name|function|tool|tool_name|function_name)>',
                body,
                re.IGNORECASE,
            )
            if name_tag_match:
                ...
                # Check leading text before first '<'
                first_tag_idx = body.find('<')
                if first_tag_idx != -1:
                    leading_text = body[:first_tag_idx].strip()
                else:
                    leading_text = body.strip()
                if leading_text:
                    token = leading_text.split()[0].strip()
                    if re.match(r'^[a-zA-Z0-9_.:-]+$', token):
                        tool_name = token

            # Extract key-value pairs
            # Format A: <arg_key>...</arg_key>\s*<arg_value>...</arg_value>
            kv_matches = re.findall(
                r'<(?:arg_key|key|parameter_name|param_name)>\s*(.*?)\s*</(?:arg_key|key|parameter_name|param_name)>\s*<(?:arg_value|value|parameter_value|param_value)>\s*(.*?)\s*</(?:arg_value|value|parameter_value|param_value)>',
                body,
                re.DOTALL | re.IGNORECASE,
            )
            for k, v in kv_matches:
                k = k.strip()
                if k:
                    params[k] = _parse_arg_value(v)

            # Format B: <parameter name="...">...</parameter> or <param name="...">...</param> or <arg name="...">...</arg>
            param_named_matches = re.findall(
                r'<(?:parameter|param|arg)\s+name\s*=\s*["\']([^"\']+)["\'](?:\s*value\s*=\s*["\']([^"\']*)["\'])?\s*(?:/>|>(.*?)</(?:parameter|param|arg)>)',
                body,
                re.DOTALL | re.IGNORECASE,
            )
            for p_name, p_attr_val, p_body_val in param_named_matches:
                p_name = p_name.strip()
                if p_name and p_name not in params:
                    val = p_attr_val if p_attr_val else p_body_val
                    params[p_name] = _parse_arg_value(val or '')

        tool_name = resolve_tool_name(tool_name)
        if tool_name:
            args_str = (
                JSONCodec.dumps(params, ensure_ascii=False)
                if isinstance(params, dict)
                else (str(params) if params else '{}')
            )
            results.append({'name': tool_name, 'arguments': args_str})

    # If no XML tags found, check for standalone JSON tool calls if available_tools is present
    if not results:
        code_block_matches = re.findall(r'```(?:json)?\s*([\[\{].*?[\]\}])\s*```', text, re.DOTALL)
        candidate_json_strings = code_block_matches + [text]
        for cjs in candidate_json_strings:
            first_brace = cjs.find('{')
            last_brace = cjs.rfind('}')
            if first_brace != -1 and last_brace != -1 and last_brace > first_brace:
                try:
                    data = JSONCodec.loads(cjs[first_brace : last_brace + 1])
                    if isinstance(data, dict):
                        if 'tool_calls' in data and isinstance(data['tool_calls'], list):
                            for tc in data['tool_calls']:
                                if isinstance(tc, dict):
                                    tname = resolve_tool_name(tc.get('name') or tc.get('function'))
                                    targs = tc.get('parameters') or tc.get('arguments') or tc.get('args') or {}
                                    if tname:
                                        targs_str = (
                                            JSONCodec.dumps(targs, ensure_ascii=False)
                                            if isinstance(targs, dict)
                                            else str(targs)
                                        )
                                        results.append({'name': tname, 'arguments': targs_str})
                        elif 'name' in data and ('parameters' in data or 'arguments' in data):
                            tname = resolve_tool_name(data.get('name'))
                            targs = data.get('parameters') or data.get('arguments') or {}
                            if tname:
                                targs_str = (
                                    JSONCodec.dumps(targs, ensure_ascii=False)
                                    if isinstance(targs, dict)
    ...
    try:
        ...
        try:
            try:
                ...
                extracted = extract_tool_calls_from_text(raw_content, available_tools=tools)
                if extracted:
                    result = {
                        'tool_calls': [
                            {
                                'name': ec['name'],
                                'parameters': (
                                    JSONCodec.loads(ec['arguments'])
                                    if isinstance(ec['arguments'], str)
                                    else ec['arguments']
                                ),
                            }
                            for ec in extracted
        ...
        async def response_handler(response, events):
            ...
            try:
                ...
                async def stream_body_handler(response, form_data):
                    ...
                    # Fallback path: extract tool calls from reasoning / think blocks or message content
                            ...
                            if item_type == 'reasoning':
                                for part in (item.get('content', []) or []):
                                    if isinstance(part, dict) and part.get('type') == 'output_text':
                                        text_sources.append(part.get('text', ''))
                                    elif isinstance(part, dict) and 'text' in part:
                                        text_sources.append(part.get('text', ''))
                                for detail in (item.get('reasoning_details', []) or []):
                                    if isinstance(detail, dict) and detail.get('text'):
                                        text_sources.append(detail.get('text', ''))
                            elif item_type == 'message':
                                for part in (item.get('content', []) or []):
                                    if isinstance(part, dict) and part.get('type') == 'output_text':
                                        text_sources.append(part.get('text', ''))
                            ...
                            if combined_text:
                                ...
                                for pc in parsed_calls:
                                    ...
                                    extracted_tool_calls.append(
                                        {
                                            'id': call_id,
                                            'index': len(extracted_tool_calls),
                                            'function': {
                                                'name': pc['name'],
